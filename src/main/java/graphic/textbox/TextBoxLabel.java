...
public abstract class TextBoxLabel extends TextBox implements Observer {
  ...
  /**
   * Compute the second point of the line drawn between the label and the component associated.
   *
  ...
  /**
   * This method compute the new location of the label according to its associated component. The location of the label
   * is relative to its component. If, for exemple, the label is x = 10 and y = 20 compared to its component and this
   * component moved, the label will compute it's location in regard to the new component bounds but always with a
   * movement of x = 10 and y = 20.
   */
  ...
  public void gMouseDragged(MouseEvent e) {
    ...
    setBounds(new Rectangle(bounds.x + mouse.x - mousePosition.x, bounds.y
                                                                  + mouse.y - mousePosition.y, 0, 0));
  ...
  public Rectangle getBounds() {
    ...
    if (isSelected() && bounds.width == 0) bounds.width = 30;
    return bounds;
  }
