 ...
 * is a copy of the component show when the component is resized or moved. When the apply method is called, the bound of
 * the component are translate by the bounds of the ghost.
 *
...
public abstract class MovableComponent extends GraphicComponent {
  ...
  public MovableComponent(GraphicView parent) {
    ...
    setLeftResizer(new GripEntity(parent, resizerSize, Cursor.W_RESIZE_CURSOR) {
      ...
      public void gMousePressed(MouseEvent e) {
        if (!g.isSelected()) parent.unselectAll();
        g.setSelected(true);
  ...
  public void apply(MouseEvent e) {
    ...
    for (final LineView line : parent.getLinesViewAssociedWith(this))
      ...
      if (line.getFirstPoint().getAssociedComponentView().equals(this)
          && line.getLastPoint().getAssociedComponentView().isSelected())
  ...
  /**
   * Compute the location of the resize. The offset define if it's the right or the left resizer. The offset is add to
   * the bounds.x location. The y location is the middle bounds.y (bounds.y + bounds.heigth / 2)
   *
   * @param offset value to add at the bounds.x location (0 for left, bounds.width for right).
   *
  ...
  public Rectangle computeLocationResizer(int offset) {
    ...
    return new Rectangle(bounds.x - size / 2 + offset, bounds.y
  ...
  public void resizeLeft(MouseEvent e) {
    /**
     * Compute the new size of the ghost and the new position of left resizer
     * from the buffered mouse location and the current mouse location. The
     * bounds of a rectangle is the location of the left corner plus two size.
     * For resize from left, we can't just grow width and height, but move its
     * left corner location too.
     */
    final Rectangle saveGhost = new Rectangle(ghost);
    ghost = new Rectangle(getBounds());
  ...
  public void resizeRight(MouseEvent e) {
    /**
     * Compute new size from right resize is quit easy than left resize. We just
     * have to grow it's width. This method compute a new location for the right
     * resizer too.
     */
    final Rectangle saveGhost = new Rectangle(ghost);
    ghost = new Rectangle(getBounds());
    ...
    ghost.width = ajustOnGrid(boundsResizer.x - ghost.x)
                  + GraphicView.getGridSize();

    if (ghost.width < MINIMUM_SIZE.x) ghost.width = saveGhost.width;
  ...
  /**
   * Set a left resizer. A resizer is used to resizer graphic component bounds. The left resizer resize the component
   * from left.
   *
   * @param left the new resizer.
   */
  public void setLeftResizer(GripEntity left) {
    if (left == null) throw new IllegalArgumentException("left is null");

    if (leftMovableSquare != null) parent.removeComponent(leftMovableSquare);
   ...
   *
   * @param visible true for showing resizers; false for hide them.
   */
  ...
  /**
   * Set a right resizer. A resizer is used to resizer graphic component bounds. The right resizer resize the component
   * from right.
   *
   * @param right the new resizer.
   */
  public void setRightResizer(final GripEntity right) {
    if (right == null) throw new IllegalArgumentException("right is null");

    if (rightMovableSquare != null) parent.removeComponent(rightMovableSquare);
