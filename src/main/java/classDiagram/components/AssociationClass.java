...
public class AssociationClass extends ClassEntity {
  ...
  /**
   * Construct a new association class. An association class is a class in UML structure associated with an association.
   * Does not construct a new binary association but will use the binary given in parameters.
   *
   * @param name the name of the class
   * @param visibility the visibility of the class
   * @param binary the binary association associated with the association class
   */
  ...
  /**
   * Construct a new association class. An association class is a class in UML structure associated with an association.
   * Does not construct a new binary association but will use the binary given in parameters. Does not generate a new
   * unique id but will use the id given in parameters.
   *
   * @param name the name of the class
   * @param visibility the visibility of the class
   * @param binary the binary association associated with the association class
   * @param id the id to given to the class
   */
  public AssociationClass(String name, Visibility visibility, Binary binary,
                          int id) {
  ...
  /**
   * Construct a new association class. An association class is a class in UML structure associated with an association.
   * A new binary association will be created between the given source and target.
   *
   * @param name the name of the class
   * @param visibility the visibility of the class
   * @param source the source of the new association.
   * @param target the target of the new association.
   */
  public AssociationClass(String name, Visibility visibility, Entity source,
                          Entity target) {
    super(name, visibility);
  ...
  private void checkAssociationCreation(Binary binary) {
    if (binary == null)
      throw new IllegalArgumentException("Creation of association " + name
                                         + " failed because binary is null.");
  ...
  private void checkAssociationCreation(Entity source, Entity target) {
    if (source instanceof AssociationClass
        || target instanceof AssociationClass)
      throw new IllegalArgumentException(
          "Creation of association "
          + name
          + " failed.\nAn association class cannot be associed with another association class.");
  ...
  protected String getLastBalise(int depth) {
    ...
    final String xml = tab + "<associationClassID>" + association.getId()
                       + "</associationClassID>\n";
  ...
  public Element getXmlElement(Document doc) {
    Element entity = super.getXmlElement(doc), associationClassID = doc
        .createElement("associationClassID");
   ...
   *
   * @param binary the new association
   */
