...
/**
 * Abstract class containing all classes parameters (attributes, methods, visibility, ...)
 *
...
public abstract class Entity extends Type implements Cloneable, Transferable {
  ...
  final public static DataFlavor ENTITY_FLAVOR = new DataFlavor(Entity.class, "Entity Type");
  ...
  public Entity clone() throws CloneNotSupportedException {
    try {
      ...
      String classToInstanciate = getClass().equals(AssociationClass.class)
          ? ClassEntity.class.getName()
          : getClass().getName();
      Entity entity = (Entity) Class.forName(classToInstanciate).getConstructor(String.class).newInstance(getName());
      ...
      SMessageDialog.showErrorMessage("An error occured when copying the entity.\nPlease send a report.");
      Slyum.LOGGER.log(Level.SEVERE, "An error occured when copying the entity.", e);
    }
    return null;
  }
  ...
  /**
   * Return all entities associed with this one. Associed means that there is a relation between this entity and another
   * one.
   *
  ...
  public HashMap<Relation, Entity> getLinkedEntities() {
    ...
    for (Relation relation : PanelClassDiagram.getInstance().getClassDiagram().getRelations()) {

      if (relation.getSource() == this) entities.put(relation, relation.getTarget());

      else if (relation.getTarget() == this) entities.put(relation, relation.getSource());

      else if (relation instanceof Multi) entities.put(relation, null);
    }
    return entities;
  }
  ...
  public void setStereotype(String stereotype) {
    if (stereotype == null) throw new IllegalArgumentException("stereotype is null");
   ...
   *
   * @param depth the number of tabs to add before each tag
   *
  ...
  /**
   * Move the object's position in the given array by the given offset. Offset is added to the current index to compute
   * the new index. The offset can be positive or negative.
   *
   * @param <T> the type of the components.
   * @param list the list containing the object to move
   * @param o the object to move
   * @param offset the offset for compute the new index
   */
  protected <T> void moveComponentPosition(final List<T> list, final T o, final int offset) {
