...
public class Type extends Observable implements IDiagramComponent {
  ...
  private final static String REGEXP_GENERIC =
      Variable.REGEX_SEMANTIC_ATTRIBUTE + "(<(" + Variable.REGEX_SEMANTIC_ATTRIBUTE + ")(,\\s*(" +
      Variable.REGEX_SEMANTIC_ATTRIBUTE + "))*>)?";
  private final static String REGEXP_GENERIC_2 = "(<(" + REGEXP_GENERIC + ")(,\\s*(" + REGEXP_GENERIC + "))*>)";
  public final static String REGEX_SEMANTIC_TYPE =
      Variable.REGEX_SEMANTIC_ATTRIBUTE + REGEXP_GENERIC_2 + "?(\\[" + REGEX_DIGIT + "])*";
  ...
  private void initialize(String name) {
    if (!TypeName.getInstance().verifyName(name)) throw new IllegalArgumentException("semantic incorrect");
  ...
  public boolean setName(final String name) {
    if (!TypeName.getInstance().verifyName(name)) return false;
    ...
    for (int i = 0; i < name.length(); i++) {
      ...
      switch (state) {
        ...
        case 1:
          ...
          if (c == ']') {
            int s = -1;
            if (!buff.isEmpty()) s = Integer.parseInt(buff);
    ...
    for (int size : a)
      if (!dimensionSet && size >= 0) return false;
      else dimensionSet &= size >= 0;
  ...
  /**
   * Return the number of dimension of this type, or 0 if the type doesn't represent an array.
   *
   * @return the number of dimension of this type, or 0 if the type doesn't represent an array.
   */
  ...
  /**
   * Return a LinkedList containing the size of the dimensions in the array. If the LinkedList is empty, that means this
   * type doesn't represent an array. The number -1 means the size is left blank (only possible in last dimension).
   *
