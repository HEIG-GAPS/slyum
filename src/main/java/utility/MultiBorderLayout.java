...
/**
 * Extends BorderLayout with multiple components in the northList, southList, eastList, westList and centerList. Layout
 * is used for correct working multiple toolbars.
 *
  ...
  private Dimension layoutSize(final Container target,
    ...
    synchronized (target.getTreeLock()) {
      ...
      final Dimension dim = new Dimension(
          //
          max(northSize.width, southSize.width, westSize.width
                                                + centerSize.width + eastSize.width) //
          ,
          northSize.height
  ...
  public void layoutContainer(final Container target) {
    synchronized (target.getTreeLock()) {
      ...
      for (final Component c : northList) {
        if (c.isVisible()) {
          final Dimension d = sumHorizontal(java.util.Arrays.asList(c),
                                            getPreferredSize);
      ...
      for (final Component c : westList) {
        if (c.isVisible()) {
          final Dimension d = sumVertical(java.util.Arrays.asList(c),
                                          getPreferredSize);
          c.setBounds(left, top, d.width, d.height);
          top += d.height;
        }
