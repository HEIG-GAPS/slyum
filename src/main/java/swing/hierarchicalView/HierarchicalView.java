...
/**
 * This class is a hierarchical view of the class diagram. It represents class diagram like a tree with all component
 * include in. JTree Swing component is used to. It implements IComponentsObserver to see changes in class diagram.
 *
...
public class HierarchicalView
  ...
  /**
   * Return the node associated with the given UML object. Return null if no associated object are found.
   *
  ...
  private static void quickSort(DefaultMutableTreeNode node, int low, int high) {
    ...
    while (i <= j) {
      ...
      while (((DefaultMutableTreeNode) node.getChildAt(i)).getUserObject().toString().compareToIgnoreCase(pivot) < 0)
        ++i;
      while (((DefaultMutableTreeNode) node.getChildAt(j)).getUserObject().toString().compareToIgnoreCase(pivot) > 0)
        --j;
  ...
  /**
   * Create a new hierarchical view of the specified class diagram. The new view is empty, if class diagram had already
   * components, you must add them manually.
   *
   * @param classDiagram the class diagram for constructing the hierarchical view.
   */
  ...
  public void addAggregation(Aggregation component) {
    addAssociation(component, "aggregation.png");
  }

  /**
   * Add the specified association in associations node.
   *
   * @param component the new association
   * @param imgPath the icon representing the association in JTree
   */
  ...
  public void addBinary(Binary component) {
    addAssociation(component, "association.png");
  }
  ...
  public void addComposition(Composition component) {
    addAssociation(component, "composition.png");
  }
  ...
  public void addMulti(Multi component) {
    addAssociation(component, "multi.png");
  }

  /**
   * Add a new node in the specified parent node.
   *
   * @param leaf the new node to add
  ...
  /**
   * Search in the entire structure of JTree the node associated with the given UML object. Return null if no associated
   * object are found.
   *
   * @param o the object associated with a node
   *
