...
/**
 * Show the panel containing all views (hierarchical, properties and graphic) and the tool bar.
 *
...
public class PanelClassDiagram extends JPanel {
  ...
  private PanelClassDiagram() {
    ...
    splitInner = new SSplitPane(JSplitPane.VERTICAL_SPLIT, STab.getInstance(), PropretiesChanger.getInstance());

    splitInner.setResizeWeight(1.d);

    // Construct outer split pane.
    splitOuter = new SSplitPane(JSplitPane.HORIZONTAL_SPLIT, hierarchicalView, splitInner);
    ...
    // Escape key
    getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT).put(KeyStroke.getKeyStroke("ESCAPE"), "escapePressed");
    ...
    getActionMap().put("newAttributePressed", new AbstractAction() {
      ...
      @Override
      public void actionPerformed(ActionEvent e) {
        List<EntityView> selectedEntities = MultiViewManager.getSelectedGraphicView()
                                                            .getSelectedEntities()
                                                            .stream()
                                                            .filter(
                                                                entity -> entity.getComponent() instanceof SimpleEntity)
   ...
   *
   * @return the {@link JOptionPane} option value.
   */
  public int askSavingCurrentProject() {
    if (!Change.hasChange()) return JOptionPane.NO_OPTION;
    else return SMessageDialog.showQuestionMessageYesNoCancel("Save current project ?");
  }
  ...
  /**
   * Remove all not associated with view components.
   *
   * @return the amount of components that have been cleaned.
   */
  ...
  public void exportAsImage() {
    ...
    fc.addChoosableFileFilter(new FileFilter() {
      ...
      public boolean accept(final File f) {
        ...
        if (extension != null)
          if (extension.equals("jpg") || extension.equals("png") || extension.equals("gif")) return true;
    ...
    if (result == JFileChooser.APPROVE_OPTION) {
      ...
      if (extension == null) file = new File(file.getPath() + ".png");
  ...
  public void exportAsVectoriel(final String selectedExtension, final String... extensions) {
    ...
    for (String extension : extensions) {
      ...
      if (extension.equals(selectedExtension)) fc.setFileFilter(ff);
    }
    ...
    if (result == JFileChooser.APPROVE_OPTION) {
      ...
      if (extension == null) file = new File(file.getPath() + fc.getFileFilter().toString());
  ...
  public void exportFileTo(File file) {
    try {
      ...
      if (file.exists()) if (SMessageDialog.showQuestionMessageOkCancel(file + " already exists. Overwrite?") ==
                             JOptionPane.CANCEL_OPTION) return;

      GraphicView graphicView = MultiViewManager.getSelectedGraphicView();
      switch (extension) {
        case "png":
          ImageIO.write(ExportViewImage.create(graphicView).export(), extension, file);
          break;
        case "jpg":
        case "gif":
          ImageIO.write(ExportViewImage.create(graphicView, BufferedImage.TYPE_INT_RGB).export(), extension, file);
          break;
        case "pdf":
  ...
  public void setCurrentFile(File file) {
    ...
    if (Slyum.isCleanAtOpeningEnable()) PanelClassDiagram.cleanComponents();
    ...
    try {
      ...
      Logger.getLogger(PanelClassDiagram.class.getName()).log(Level.SEVERE, "Unable to register file", ex);
    }
  ...
  /**
   * Init a new save where save project. If no file exist, open a JFileChooser to ask a new file.
   *
  ...
  public void setDisabledUpdate(boolean disabledUpdate) {
    if (this.disabledUpdate == disabledUpdate) return;
  ...
  public void openFromXML() {
    ...
    if (result == JFileChooser.APPROVE_OPTION) openFromXML(fc.getSelectedFile());
  }
  ...
  public void saveSplitLocationInProperties() {
    ...
    dividerLocationBottom =
        (float) splitInner.getDividerLocation() / (float) (splitInner.getHeight() - splitInner.getDividerSize());
    properties.put(PropertyLoader.DIVIDER_BOTTOM, String.valueOf(dividerLocationBottom));

    dividerLocationLeft =
        (float) splitOuter.getDividerLocation() / (float) (splitOuter.getWidth() - splitOuter.getDividerSize());
    properties.put(PropertyLoader.DIVIDER_LEFT, String.valueOf(dividerLocationLeft));
   ...
   *
   * @param selectFile true if a dialog must invite the user to choose a file; false to save in current file. If no
   * current file, dialog will open.
   */
  public void saveToXML(boolean selectFile) {
    if (selectFile || currentFile == null || !currentFile.exists()) if (!initCurrentSaveFile()) return;
  ...
  public static void saveDocumentInCurrentFile(Document document, File currentFile) throws
    ...
    try {
      ...
      Logger.getLogger(PanelClassDiagram.class.getName()).log(Level.SEVERE, "Unable to register file", ioe);
    }
  }
  ...
  /**
   * Remove all tabs, nodes and graphicviews from this PanelClassDiagram. Do not remove them from current file. This
   * method is generally used with by cleanApplication when opening a new file.
   */
  ...
  private FileFilter createChoosableFileFilter(final String extension) {
    return new FileFilter() {
      ...
      public boolean accept(File f) {
        ...
        if (fileExtension != null) if (fileExtension.equals(extension)) return true;
  ...
  private class FileHandler extends TransferHandler {
    ...
    public boolean importData(JComponent comp, Transferable t) {
      try {
        @SuppressWarnings("unchecked") List<File> dropppedFiles = (List<File>) t.getTransferData(
  ...
  private class SlyFileChooser extends FileFilter {
    ...
    public boolean accept(File f) {
      ...
      if (extension != null) if (extension.equals(Slyum.EXTENTION)) return true;
