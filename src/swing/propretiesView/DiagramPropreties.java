...
public class DiagramPropreties extends GlobalPropreties {
  ...
  public static DiagramPropreties getInstance() {
    if (instance == null)
      instance = new DiagramPropreties();
    return instance;
  }
  ...
  public void updateComponentInformations(UpdateMessage msg) {
    ...
    if (fileOpen != null) {
      ...
      lblFileName.setText(
          String.format(
              "Name: %s", 
              fileOpen.getName().replaceAll(Slyum.FULL_EXTENTION, "")));
      
      lblFileAbsolutePath.setText(
          String.format("Path: %s", fileOpen.getAbsolutePath()));
    } else {
    ...
    String textDefault = String.format(
        statistics,
        ...
        cd.countComponents(Composition.class),
        gv.countNotes()).replaceAll(regexRemoveLineWithZero, "");
    
     String textSelection = String.format(
        statistics,
