...
public class MultiFactory extends CreateComponent
{
	...
	public MultiFactory(final GraphicView parent) {
		...
		buttons[0] = new FlatButton(PersonalizedIcon.createImageIcon(Slyum.ICON_PATH + "check-mark.png"));
		...
		buttons[1] = new FlatButton(PersonalizedIcon.createImageIcon(Slyum.ICON_PATH + "delete-24.png"));
    ...
    buttons[1].setBounds(buttons[0].getX() + buttons[0].getWidth() + 10, buttons[0].getY(), bw, bh);
	 ...
	 */
	private LinkedList<ClassEntity> getClassEntity(
	    LinkedList<ClassView> classView) {
	...
	public void paintComponent(Graphics2D g2) {
		Rectangle bounds = Utility.scaleRect(
		    parent.getScene().getVisibleRect(), parent.getInversedScale());
		...
		for (ClassView cv : classSelected) {
			...
			g2.drawRect(cvBounds.x, cvBounds.y,
			    cvBounds.width + 1, cvBounds.height + 1);
		}
